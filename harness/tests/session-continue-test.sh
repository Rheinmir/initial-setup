...
# phiên "s-over": 600 lượt — vượt per_session_model_calls 500 mặc định ($6 KHÔNG đủ kích hoạt: trần $ tạm tắt)
printf '{"s-over":{"session":"s-over","turns":600,"tokens":{"input_tokens":100,"output_tokens":100},"cost_usd":6.0},"s-usd":{"session":"s-usd","turns":12,"tokens":{"input_tokens":100,"output_tokens":100},"cost_usd":60.0},"s-ok":{"session":"s-ok","turns":2,"tokens":{"input_tokens":50,"output_tokens":50},"cost_usd":0.1}}' > "$TMP/harness/metrics/cost-by-session.json"
...
rc=$(run_hook s-usd "làm việc B"); [ "$rc" = 0 ] && echo "  ✓ chỉ vượt \$ (60 > 5) → KHÔNG chặn (trần \$ tạm tắt)" || { echo "  ✗ trần \$ vẫn chặn rc=$rc"; cat /tmp/sc-err; exit 1; }
...
for k in "Last user prompt: prompt bị chặn X" "Read the complete original session transcript" "$TMP/t.jsonl" "historical reference data" "git status" "Briefly state where the previous session stopped"; do grep -q "$k" "$HF" && echo "  ✓ bàn giao có: $k" || { echo "  ✗ bàn giao thiếu: $k"; exit 1; }; done
