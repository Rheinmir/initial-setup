...
| **Skill security / supply-chain (GH#13 — ĐÍNH CHÍNH: đã CLOSED/COMPLETED 04/07, ledger vừa sửa lại đúng)** | `orca-sec-scans` (Trivy), `/skill-provenance` (sha256), `egress-guard.py` (chặn net-egress + MCP-injection-pattern, self-test PASS **và giờ đã nối vào `pre_tool_use.py` PreToolUse thật cho mọi lệnh Bash — commit `b2b7b08`**) — 84 skill **private**, không kéo marketplace mở | 🔴 **CVE-2026-25253** — CVE ĐẦU TIÊN cấp cho agentic-AI skill runtime (RCE OpenClaw, 27/01/2026). Mitiga Labs (07/2026) trình diễn skill "hợp pháp trông thấy" âm thầm **exfiltrate toàn bộ codebase** — vector cụ thể chưa nêu ở các kỳ trước, đúng lớp mà `egress-guard.py` được thiết kế để bắt (network egress không nằm trong allow-list) | Ngang có-cơ-chế — **đã sửa xong phần wiring trong lần rà này, còn đúng 1 việc** | ⚠️ **Tự-đính-chính:** bản viết đầu của dòng này (và comment đầu GH#13) nói sai — trước bản vá `b2b7b08`, `egress-guard.py` CHƯA từng chạy lúc runtime (chỉ `--self-test` trong medic/fdk-gate), nên "chỉ cần lật `mode: block`" là claim thiếu bước wiring. Đã sửa: `pre_tool_use.py` giờ gọi egress-guard cho mọi Bash event thật (test trực tiếp: rc=0 lệnh vô hại, rc=0 lệnh curl khả nghi vì còn `mode: warn`). Việc CÒN LẠI đúng 1 bước: calib `allow_domains` thật trong `harness/egress-guard.config.yaml` rồi lật `mode: block` — giờ lật mới có tác dụng thật |
...
1. **GH#13 (supply-chain, đã closed, KHÔNG mở lại):** ĐÃ SỬA trong lần rà này (commit `b2b7b08`) — `egress-guard.py` trước đó chỉ chạy `--self-test`, chưa từng gate Bash thật lúc runtime; giờ đã nối vào `pre_tool_use.py`. Còn đúng 1 việc: calib `allow_domains` thật trong `harness/egress-guard.config.yaml` rồi lật `mode: block` (hiện `warn`, fail-open có chủ đích).
