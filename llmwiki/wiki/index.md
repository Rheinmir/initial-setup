...
| [010826-wiki-mental-model-taxonomy](sources/draft/010826-wiki-mental-model-taxonomy.md) | draft | SPEC GH#93: quan hệ đồ thị mới `operationalizes` (suy sống, không khai tay) nối wiki node `layer: fact\|mental-model` tới skill/rule Protocol, kèm đường tắt Fact→Protocol và visualize badge cho người học mới |
| [010826-wiki-mental-model-taxonomy-PLAN](sources/draft/010826-wiki-mental-model-taxonomy-PLAN.md) | plan | PLAN thi hành GH#93: 4 task tracer-bullet — hàm suy `operationalizes_targets()` (wiki-graph.py), wire hiển thị + badge layer (build-wiki-graph.py), gắn layer mẫu, medic gate |
| [030826-wiki-layer-suggest](sources/draft/030826-wiki-layer-suggest.md) | draft | SPEC: script heuristic report-only gợi ý `layer: fact\|mental-model` cho dự án cũ (không tự ghi frontmatter), mỗi gợi ý kèm `layer_source: heuristic` + `layer_date` để review/ghi-đè sau |
