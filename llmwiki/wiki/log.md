...
## 2026-08-01 — propose — wiki-mental-model-taxonomy (GH#93)
Draft SPEC cho issue [Rheinmir/setup#93](https://github.com/Rheinmir/setup/issues/93): quan hệ đồ thị mới `operationalizes` nối wiki node phân loại `layer: fact|mental-model` tới Protocol (skill/rule) mà nó tạo ra, suy sống (infer-at-build) theo đúng nguyên tắc "suy đừng cất" đã học từ lịch sử `touches` — không khai tay trong `relations:`, tránh lặp lại bug đóng-băng-0,8%-coverage đã ghi trong `[[wiki-core-relations]]`. Cho phép đường tắt Fact→Protocol trực tiếp theo yêu cầu bổ sung của tác giả issue (comment #2, tránh chính taxonomy gây phình hệ thống). Ledger gốc mà issue trích dẫn không tồn tại trong repo (0 kết quả mọi branch) — đã hỏi lại user, dựng SPEC từ issue body + 2 comment cập nhật. STOP chờ duyệt, chưa code.

## 2026-08-01 — plan — wiki-mental-model-taxonomy-PLAN (GH#93)
