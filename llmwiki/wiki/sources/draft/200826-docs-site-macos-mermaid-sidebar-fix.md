...
- `registry.npmjs.org/beautiful-mermaid` (fetch qua WebFetch 2026-08-20) — version 1.1.3, unpacked 2.1MB, deps `elkjs@^0.11.0` + `entities@^7.0.1`, main entry `dist/index.js` (ESM). Chưa xác nhận có bản browser-IIFE dựng sẵn hay phải tự bundle — ghi thành `[[200826-docs-site-macos-mermaid-sidebar-fix]] U-01`.
...
- Cách đóng gói `beautiful-mermaid` thành browser bundle `(default, find-out-later → [[200826-docs-site-macos-mermaid-sidebar-fix]] U-01)` — giả định cần một bước `esbuild --bundle --format=iife` chạy MỘT LẦN off-band, chưa xác nhận package có sẵn bản browser-ready hay không.
...
- **Unknown ghi nợ, đã trả:** [[200826-docs-site-macos-mermaid-sidebar-fix]] U-01 — resolved 2026-08-20, bằng chứng là PoC thật `llmwiki/html/200826-beautiful-mermaid-poc.html` (verify bằng Playwright/Chromium: `node /tmp/bm-poc/verify.mjs`, ảnh chụp `/tmp/bm-poc/shot-{light,dark,zoomed}.png`).
- **Task ID:** `T-260820-01`.
